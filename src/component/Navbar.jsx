...
const Navbar = () => {
  ...
  return (
    <nav className='flex flex-wrap justify-between md:items-center text-white px-10 pt-6 md:px-20'>
      ...
      {showmenu?(
        <RiMenu2Line size={30} className='md:hidden absolute right-10 top-6 transiton-all duration-300' onClick={()=>{openmenu(!menu); setshowmenu(!showmenu);}}/>
      ):(
        <RiCloseLine size={30} className='md:hidden absolute right-10 top-6 transiton-all duration-300'/>
      )}
    </nav>
  )
}
